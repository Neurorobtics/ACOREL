...
if nargout==0
   ...
   if dim==3
       ...
       quiver3(x,y,z,T(:,1),T(:,2),T(:,3),'color','k'),hold on
       if nargin>3
           initr=repmat(init,[1 size(x,1)])';
           quiver3(x,y,z,initr(:,1),initr(:,2),initr(:,3),'color','y'),hold on
           legend('Curve','Normal','Binormal','Tangent','Initial')
       else
       ...
       grid,daspect([1 1 1]),axis vis3d
       legend('Tangent','Normal','Binormal','Tangent')
   end
end
